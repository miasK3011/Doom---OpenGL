...
static GLvoid * font_bitmap = GLUT_BITMAP_HELVETICA_18;
static GLvoid * font_stroke = GLUT_STROKE_ROMAN;
...
namespace my
{
  ...
  //usada a to_string do namespace std se a versao for maior ou igual ao C++11
#else
 ...
 */
inline void draw_text_bitmap(float x, float y, std::string text, void * font = font_bitmap)
{
    glRasterPos2f(x, y);
 ...
 */
inline void draw_text_stroke(float x, float y, std::string text, float scale = SCALE, float lineWidth = LINE_WIDTH, void * font = font_stroke)
{
    glPushMatrix();
        glTranslatef(x, y, 0);
